...
const Features = () => {
  return (
    <section>
      {/* Scrollable anchor element to jump to "features" section */}
      <Element name="features">
        <div className="container">
          {/* Main container for feature items with responsive styles */}
          <div className="relative flex md:flex-wrap flex-nowrap border-2 border-s3 rounded-7xl md:overflow-hidden max-md:flex-col feature-after md:g7 max-md:border-none max-md:rounded-none max-md:gap-3">
            {/* Mapping through each feature from constants to display feature blocks */}
            {features.map(({ id, icon, caption, title, text, button }) => (
              ...
              >
                ...
                {/* Button component with icon, passed as children */}
                <Button icon={button.icon}>{button.title}</Button>
              </div>
            ))}

            {/* Horizontal details list shown only on non-mobile devices */}
            <ul className="relative flex justify-around flex-grow px-[5%] border-2 border-s3 rounded-7xl max-md:hidden">
              <div className="absolute bg-s3/20 top-[38%] left-0 right-0 w-full h-[1px] z-10" />
              {/* Mapping through details array to display additional icons and titles */}
              {details.map(({ id, icon, title }) => (
                <li key={id} className="relative pt-16 px-4 pb-14">
                  ...
                  {/* Title for each icon in details list */}
